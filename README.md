...
### Who is PlanOut for?
This software release is a Python implementation of PlanOut designed for researchers, students, and small startups wanting to run experiments. It is designed to be extensible, so that it may be adapted for use with large production environments.  At Facebook, we use a separate implementation of PlanOut that is integrated with our production services to conduct experiments with hundreds of millions of users.
